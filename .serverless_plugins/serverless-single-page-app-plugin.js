...
class ServerlessPlugin {
    constructor(serverless, options) {
        ...
        this.commands = {
            syncToS3: {
                usage: 'Deploys the `custom.s3LocalPath` directory to your bucket',
                lifecycleEvents: [
                    'sync',
                ],
            },
    ...
    // syncs the custom.s3LocalPath directory to the provided bucket
    syncDirectory() {
        const s3Bucket = this.serverless.variables.service.custom[this.options.stage].s3Bucket;
        const s3LocalPath = this.serverless.variables.service.custom.s3LocalPath;
        const args = [
            '--profile',
            this.serverless.variables.service.custom[this.options.stage].profile,
            's3',
            'sync',
            `${s3LocalPath}${path.sep}`,
            `s3://${s3Bucket}/`,
        ];
